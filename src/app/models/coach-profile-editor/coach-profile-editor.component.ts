...
export class CoachProfileEditorComponent implements OnInit {
  ...
  tables$: Observable<Question[]> | Subject<Question[]> = this.coachEdit.questions$;
  ...
  tabChanged(tabChangeEvent: MatTabChangeEvent): void {
    if (tabChangeEvent.index === 0) {
      ...
      this.tables$ = of(MOCK_AUDITION_QUESTIONS as any as Question[]);
    } else if (tabChangeEvent.index === 2) {
      this.selectedTab = CoachEditorTabs.writingAndSpeaking;
      this.tables$ = of(MOCK_WRITING_AND_SPEAKING_QUESTIONS as any as Question[]);
    }
  }
